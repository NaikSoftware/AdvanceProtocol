...
## Стиль, респонсивність і воркфлоу арту (наскрізне для всіх екранів 3.2–3.5)

Ці правила діють для кожного екрана оболонки, а не окремим завданням.

**Інтеграція арту — «тема + арт-акценти».** Кнопки, панелі, слайдери — це Godot-`Control`
зі `StyleBox`/`Theme` (чіткі, масштабуються під будь-який екран). 2D-арт від `agy` дає рівно
атмосферу: повноекранні фони, рамки/іконки як `ninepatch`, текстури-акценти. Жоден екран **не**
малюється цілісною картинкою з прозорими кнопками зверху — таке не тягнеться під різні
співвідношення сторін і не лишається чітким.

**Воркфлоу генерації — ворота.** Спершу **сірий скелет**: усі екрани клікабельні, навігація
працює, розкладка респонсивна, **без жодного арту** (нейтральна тема, суцільні `ColorRect`
замість фонів). Апрув UX і розкладки. **Лише після апруву** — генерація фінального арту через
`agy` в єдиному візуальному стилі й заміна заглушок на тему з текстурами. Один стильовий пробник
(1–2 екрани) апрувиться до генерації повного набору.

**Респонсивність — обов'язкова, від мобілки до ПК.** Кожен екран будується на `anchors` +
контейнерах (`MarginContainer`/`VBox`/`HBox`/`CenterContainer`), ніяких абсолютних координат.
`SafeArea`-`MarginContainer` як у `hud.tscn` (виріз/безпечні зони). Контент обмежений
`max_width`-контейнером по центру, щоб на широкому ПК-екрані меню не розтягувалось на всю ширину.
Тапабельні цілі ≥ 48 dp (§Global Constraints). Перевірка — візуально через Godot MCP на широкому
(1280×720+) і вузькому (телефон, портрет заборонений — лендскейп 720×360-типу) вьюпортах.

**Стиль — атмосферний, сучасний, військово-тактичний C2.** Низька насиченість, приглушені панелі,
тонкі яскраві акцентні лінії, конденсований/моношрифт (узгоджено з арт-дирекшеном: near-future
2030s, §8 CLAUDE.md). Ефекти/шейдери — окремо й пізніше, не в цій фазі.

**Стан арту зараз:** 3D-ассетів ще немає (Фаза 5). Тому обіцяний у Task 3.2 «статичний рендер
бойової сцени» як тло головного меню поки недосяжний — тимчасово (і, можливо, назавжди) його
замінює згенерований `agy` атмосферний фон. Це не змінює правило «батарея дорожча за анімацію»:
тло статичне.
