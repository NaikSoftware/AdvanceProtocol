...
func occupied_map() -> Dictionary:
	...
	## йти крізь visible_occupied_map() (§3.5), інакше маршрут доносить про невидимого
...
func visible_occupied_map(player: int) -> Dictionary:
	## Прохідність очима одного гравця: свої юніти — завжди, чужі — лише поки їхній
	## тайл у visible. §3.5/§3.13: маршрут планується з того, що гравець ЗНАЄ, інакше
	## він мовчки обходить невидимого ворога, і сама форма шляху стає доносом про те,
	## де той стоїть. Під час самого ходу MoveCommand не звіряється ні з нею, ні з
	## occupied_map(): кожен крок перевіряється через unit_at() у мить входу в тайл.
	##
	## Свої юніти входять безумовно, а не через visible. Живий юніт і так світить
	## власний тайл, тож ці дві умови майже завжди збігаються, — але «майже» тут не
	## годиться: visible — то кеш, перерахований востаннє з refresh_vision(), і до
	## першого start() він порожній. Знання про власну техніку від того кешу не
	## залежить, тому воно й записане прямо, а не виведене з нього.
	var out: Dictionary = {}
	for u in alive_units():
		if u.owner == player or vision[player].is_visible(u.pos):
			out[u.pos] = u.id
	return out
